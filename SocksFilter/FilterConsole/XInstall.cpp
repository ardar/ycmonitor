...
int CXInstall::InstallProvider(TCHAR *sPathName,TCHAR *sProcessName)
{
	if(IsInstalled())
	...
	_tcscpy(m_sProcessPathName, sProcessName);
	if(!SaveReg(
		REG_INSTALL_PROCESSPATH_ITEM, 
		(BYTE*)sProcessName, 
		_tcslen(sProcessName),
		HKEY_LOCAL_MACHINE, 
		REG_INSTALL_KEY, 
		REG_SZ
		)
...
)
{
	...
	__try
	{
		...
		if (RegQueryValueExA(hSubkey, sKey, 0, &dwType, pBuffer, &dwBufSize) == ERROR_SUCCESS
			&& dwType == ulType)
			return TRUE;
	}
	__finally
