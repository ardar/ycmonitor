...
class D3GetFinishedResultPack :
	public ISocketPack
{
public:
	D3ErrorCode m_errorCode;
	CArray<D3CompletedItem*> m_itemList;

	D3GetFinishedResultPack() 
	{
		m_errorCode = D3ErrorCode::ERR_Success;
	};
	virtual ~D3GetFinishedResultPack(void)
	{
	...
	virtual BOOL FromBuffer( BYTE* buf, int len )
	{
		...
		{
			D3CompletedItem* item = new D3CompletedItem();
			SocketPackUtil::ReadCompletedItem(buf, nIndex, *item);
			m_itemList.Add(item);
		}
		return TRUE;
	};
	...
	virtual BOOL ToBuffer( BYTE* buf, int maxlen, int& bufLen )
	{
		...
		*(DWORD*)(buf+bufLen) = nItemCount;
		...
		for (int i=0;i<nItemCount;i++)
		{
			D3CompletedItem* item = m_itemList.GetAt(i);
			SocketPackUtil::WriteCompletedItem(*item, buf, maxlen, bufLen);
		}
		return TRUE;
	};
